...
class GlobalBlockLookupTest extends MediaWikiIntegrationTestCase {
	...
	private function getTargetUserForGlobalBlockId( int $globalBlockId ): User {
		$target = $this->getDb()->newSelectQueryBuilder()
			->select( 'gb_address' )
			->from( 'globalblocks' )
			->where( [ 'gb_id' => $globalBlockId ] )
			->fetchField();
		return $this->getServiceContainer()->getUserFactory()->newFromNameOrIp( $target );
	}

	private function getGloballyBlockedTestUser(): User {
		return $this->getTargetUserForGlobalBlockId( 5 );
	}
	...
	/** @dataProvider provideGetGlobalBlocksForCentralIds */
	public function testGetGlobalBlocksForCentralIds( array $centralIds, int $flags, array $expectedRowIds ): void {
		$actualGlobalBlocks = GlobalBlockingServices::wrap( $this->getServiceContainer() )
			->getGlobalBlockLookup()
			->getGlobalBlocksForCentralIds( $centralIds, $flags );
		$actualGlobalBlockIds = array_map( static fn ( $globalBlock ) => $globalBlock->getId(), $actualGlobalBlocks );

		$this->assertArrayEquals(
			$expectedRowIds,
			$actualGlobalBlockIds,
			false,
			false,
			'The global blocks returned by ::getGlobalBlocksForCentralIds are not as expected.'
		);
	}

	public static function provideGetGlobalBlocksForCentralIds(): array {
		return [
			'No central IDs provided' => [ 'centralIds' => [], 'flags' => 0, 'expectedRowIds' => [] ],
			'Central IDs do not exist' => [
				'centralIds' => [ 9876678, 123980123980 ],
				'flags' => 0,
				'expectedRowIds' => [],
			],
		];
	...
	public function testGetGlobalBlocksForCentralIdsWhenOneGlobalBlockExcluded() {
		$this->testGetGlobalBlocksForCentralIds(
			[
				// We have set user IDs to be the central IDs for these tests, so no need to
				// use the CentralIdLookup
				$this->getTargetUserForGlobalBlockId( 5 )->getId(),
				$this->getTargetUserForGlobalBlockId( 12 )->getId(),
				1234,
			],
			0,
			[ 5 ]
	...
	public function testGetGlobalBlocksForCentralIdsWhenLocalDisableCheckSkipped() {
		$this->testGetGlobalBlocksForCentralIds(
			[
				$this->getTargetUserForGlobalBlockId( 5 )->getId(),
				$this->getTargetUserForGlobalBlockId( 12 )->getId(),
	...
	public function addDBDataOnce() {
		...
		$secondTestBlockedUser = $this->getMutableTestUser()->getUserIdentity();
		...
		$this->getDb()->newInsertQueryBuilder()
			...
			->row( [
				'gb_id' => 12,
				'gb_address' => $secondTestBlockedUser->getName(),
				'gb_target_central_id' => $this->getServiceContainer()
					->getCentralIdLookup()
					->centralIdFromLocalUser( $secondTestBlockedUser ),
				'gb_by_central_id' => $this->getServiceContainer()
					->getCentralIdLookup()
					->centralIdFromLocalUser( $testUser ),
				'gb_by_wiki' => WikiMap::getCurrentWikiId(),
				'gb_reason' => 'test',
				'gb_timestamp' => $this->getDb()->timestamp( '20080405060709' ),
				'gb_anon_only' => 0,
				'gb_expiry' => $this->getDb()->encodeExpiry( '20240405060709' ),
				'gb_range_start' => '',
				'gb_range_end' => '',
				'gb_create_account' => 0,
				'gb_autoblock_parent_id' => 0,
		...
		$this->getDb()->newInsertQueryBuilder()
			...
			->row( [
				'gbw_by' => $testUser->getId(),
				'gbw_by_text' => $testUser->getName(),
				'gbw_reason' => 'test-override-2',
				'gbw_expiry' => $this->getDb()->encodeExpiry( '20240405060709' ),
				'gbw_id' => 12,
			] )
			->caller( __METHOD__ )
			->execute();
